...
from ..src.chihiros_device_manager import ble_service as api
from ..src.chihiros_device_manager import commands
from ..src.chihiros_device_manager.commands import LightWeekday
from ..src.chihiros_device_manager.commands import encoder as doser_commands
from ..src.chihiros_device_manager.device import (
    Doser,
    LightDevice,
...
_WEEKDAY_NAME_MAP = {
    name: member
    for name, member in doser_commands.PumpWeekday.__members__.items()
}
...
) -> list[doser_commands.PumpWeekday] | None:
    """Convert CLI weekday tokens into Weekday members."""
    if not weekday_names:
        return None
    resolved: list[doser_commands.PumpWeekday] = []
...
async def _prompt_weekdays() -> list[doser_commands.PumpWeekday] | None:
    ...
    try:
        while True:
            ...
            if choice == "1":
                ...
                weekdays = await _prompt_weekdays()
                if weekdays is None:
                    weekday_mask = doser_commands.encode_pump_weekdays(None)
                else:
...
def request_status(
    ...
    async def _async_func() -> None:
        status: DoserStatus | None = None
        try:
            async with api.device_session(device_address) as device:
                if not isinstance(device, Doser):
                    raise TypeError
                await device.request_status()
...
def add_setting(
    ...
    weekdays: Annotated[list[LightWeekday], typer.Option()] = [
        LightWeekday.everyday
    ],
) -> None:
